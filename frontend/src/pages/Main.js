...
const AuthMain = ({profileShow, toggleProfile}) => {
    ...
    useEffect(() => {
        const getAlarms = async () => { 
            const response = await axiosObject.get("/get-alarms");
    ...
    const handleDelete = async (index) => {
        try {
            await axiosObject.post("/delete-alarm", { eventID: events[index].EventID.S });
            ...
            <table className="table table-bordered">
                ...
                {events.map((item, index) => (
                    <tr key={index}>
                        <td>{item.Title.S}</td>
                        <td>
                            {Object.keys(item.Dates.M) && Object.keys(item.Dates.M).length !== 0?<div>
                                <h5>One Time Events:</h5>
                                <ul>
                                    {Object.values(item.Dates.M).map((event, index) => {
                                        return <li key={index}>{new Date(event.S).toLocaleString()}</li>
                                    })}
                                </ul>
                            </div>:null}
                            {Object.keys(item.Crons.M) && Object.keys(item.Crons.M).length !== 0?<div>
                                <h5>Cron Events:</h5>
                                <ul>
                                    {Object.values(item.Crons.M).map((cron, index) => {
                                        return <li key={index}>{getCronDescription(cron.S)}</li>
                                    })}
                                </ul>
                            </div>:null}
                        </td>
